...
SELECT 
	...
	DATETIME(autofill.date_created,'unixepoch') AS "Ending Timestamp"  /* Timestamp of the last record */

	FROM autofill WHERE ROWID = (SELECT MIN(ROWID) FROM autofill) AND ROWID >1 /* The first record # in the visits table is greater than 1.*/
...
UNION
	...
	(
		...
		LAG(DATETIME(autofill.date_created,'unixepoch'),1) OVER (ORDER BY ROWID) as "Beginning Timestamp", /* Gets the timestamp from the previous record */
		DATETIME(autofill.date_created,'unixepoch') AS "Ending Timestamp" /* Gets the timestamp of the current record */
		FROM autofill
	)
